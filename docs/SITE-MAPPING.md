...
質問内容が企業依存のため**機械的マッピングはしない**。代わりに「ラベル/質問文で照合する汎用フィル」で対応:

- **性別**: ラベル一致する radio を選択（例: 「男性」に `profile.gender` が一致）
- **繰り返し自由記述**: `autofillIweb` の `textRules` で、質問文にキーワードを含む空欄を補完
  - 高校名 → `profile.highSchool`（`/(出身)?高校(名|学校名)?|出身校/`）
  - 追加したい設問は `textRules` に `{ re, val }` を足す

複数企業で同じ設問が出るものは、name に依存せず**質問文ベース**で効くため横断対応できる。
